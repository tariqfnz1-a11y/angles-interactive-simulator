...
function draw(){
  ...
  const c = getCenter();
  // circle
  ctx.beginPath(); ctx.arc(c.x,c.y,radius,0,Math.PI*2); ctx.fillStyle = '#fff'; ctx.fill(); ctx.strokeStyle='#cbd5e1'; ctx.lineWidth=2; ctx.stroke();

  // fixed ray at 0° (to the right)
  const rayA = {x: c.x + radius*Math.cos(0), y: c.y + radius*Math.sin(0)};
  // moving ray at -angle (canvas y increases downward)
  const aRad = -degToRad(angle);
  const rayB = {x: c.x + radius*Math.cos(aRad), y: c.y + radius*Math.sin(aRad)};
  ...
  ctx.arc(c.x,c.y,radius*0.35, end, start, aRad>0); ctx.stroke();
  ...
  // angle text
  ctx.fillStyle = '#0f172a'; ctx.font='24px system-ui,Arial'; ctx.textAlign='center'; ctx.fillText(angle + '°', c.x, c.y - radius - 10);
}

function setAngle(deg){angle = ((deg%360)+360)%360; range.value = angle; output.textContent = angle + '°'; draw();}
